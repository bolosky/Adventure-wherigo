...
int t37() {
	...
	printMessage(2, 677, 0); if (g10(33, -1)) { l12(0, 118, 8); set_object_location(33, LOCATION_LIMBO); }
	else {
		if (g10(134, -1))
		{
		...
		if (currentCommandContains(557)) {
			...
			if (g10(7, -1)) {
				...
				if (currentCommandContains(547)) {
					...
					die();
				} if (currentCommandContains(115)) { printMessage(12, 1116, 670); die(); } modifyObjectFlag('c', 323, 8); set_object_location(24, LOCATION_LIMBO);
...
int j42()
{
	if (object_type_3_buffer[17] > 0) {
		...
		if (object_type_3_buffer[17] > 15) { 
			set_object_location(17, object_type_3_buffer[671]); 
		} 
		f3(677, 1521);
...
int n24() {
	if (currentCommandContains(101) || currentCommandContains(38)) {
		return 0;
	} l12(0, 677, 0); if (object_type_3_buffer[PLAYER_LOCATION] == LOCATION_MAGNIFICENT_VIEW) {
	...
	} if (object_type_3_buffer[PLAYER_LOCATION] == 253) { f3(677, 1209); f3(676, LOCATION_LIMBO); } if (object_type_3_buffer[PLAYER_LOCATION] == 261) {
		f3(677, 1210);
		f3(676, 260);
	...
	} if (object_type_3_buffer[PLAYER_LOCATION] == 408) { f3(677, 954); f3(676, 203); } if ((399 <= object_type_3_buffer[671] && object_type_3_buffer[671] <= 404))
	{
		...
		set_object_location(54, object_type_3_buffer[676]); if (!(item_location[54] == LOCATION_LIMBO)) { printMessage(64, 1216, 0); }
	} if (object_type_3_buffer[PLAYER_LOCATION] == 151)
	{
	...
	if (object_type_3_buffer[135] > 3)
	{
		...
		if (!(isObjectFlagSet(27, 14))) { printMessage(0, 1307, 0); } set_object_location(83, object_type_3_buffer[671]); set_object_location(135, LOCATION_MAGNIFICENT_VIEW); l12(0, 135, 4);
...
int u28() {
	if (currentCommandContains(120) || currentCommandContains(125)) {
		if (currentCommandContains(COMMAND_LOOK) || currentCommandContains(562)) {
			printMessage(64, 1397, 0); // It says the same thing as it did before.
		} 
...
int x31() {
	if (currentCommandContains(209)) {
		if (currentCommandContains(COMMAND_LOOK)) { 
			printMessage(64, 1640, 0); 
		} 
		printMessage(64, MESSAGE_RIDICULOUS, 0);
	}
} 
...
int h31() {
	...
	if (item_location[82] == 379) {
		if (isObjectFlagSet(82, 14)) {
			...
			else { set_object_location(82, LOCATION_LIMBO); printMessage(64, 1584, 0); }
		}
		else {
	...
	if (!g10(t11(object_type_3_buffer[670]), -1))
		...
		if (currentCommandContains(106)) { set_object_location(106, LOCATION_LIMBO); set_object_location(107, object_type_3_buffer[671]); set_object_location(59, 234); printMessage(64, 927, 0); }
...
int f23() {
	...
	{
		if (currentCommandContains(LOCATION_BUILDING)) { f3(679, 436); f3(678, 136); h31(); } if (currentCommandContains(144)) {
			f3(679, 142);
			f3(678, 144); h31();
		}
	}
	else { processMoveCommand(LOCATION_END_OF_ROAD, 0, -LOCATION_BUILDING); processMoveCommand(144, 0, -144); } processMoveCommand(142, 0, COMMAND_UPSTREAM, -COMMAND_NORTH);
	...
	} l12(0, 717, 6); compute_score(); printMessage(0, 1390, 0); if (y10(1391))
	{
	...
	processMoveCommand(148, 0, 148, COMMAND_ENTER, COMMAND_UP, -COMMAND_WEST); processMoveCommand(150, 0, -150); if (object_type_3_buffer[4] == 1) {
		...
		return 0; if (currentCommandContains(COMMAND_LOOK) || currentCommandContains(562)) { printMessage(64, 1614, 0); } printMessage(64, MESSAGE_RIDICULOUS, 0);
} int
l16() {
...
int y23() {
	...
	processMoveCommand(151, 0, COMMAND_NORTH, COMMAND_EXIT, -628); if (!currentCommandIsNotOneOf(125, -1))
		return 0; if (currentCommandContains(COMMAND_LOOK) || currentCommandContains(562)) { printMessage(76, 1615, 154); } printMessage(64, MESSAGE_RIDICULOUS, 0);
} 
...
int q10() {
	...
	} if (!currentCommandIsNotOneOf(COMMAND_TAKE, 11, -1)) return 0; printMessage(64, MESSAGE_DEEP_ROOTS, 0);
} int p14() {
...
int command_valuation_room_483() {
	...
	if (currentCommandContains(535) && object_type_3_buffer[697] == 2) {
		...
		if (isObjectFlagSet(t11(710), 7)) {
			printMessage(0, 1572, 0);
		} else {
			printMessage(0, 1571, 0); 
			modifyObjectFlag('s', t11(710), 7);
		} 
...
int anteroom_235() {
	...
	if (currentCommandContains(COMMAND_EAST)) { 
		modifyObjectFlag('c', t11(735), 2); processMoveCommand(LOCATION_WITTS_END, -2); 
	} 
	u28();
} 
