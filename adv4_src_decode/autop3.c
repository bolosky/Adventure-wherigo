...
int p39() {
	if (object_type_3_buffer[697] == 1) { return 0; } if (!(isObjectFlagSet(t11(670), 0))) {
		...
		} s9(object_type_3_buffer[670], PLAYER_LOCATION); if (currentCommandContains(89) && object_type_3_buffer[89] == 0) {
			modifyObjectFlag('s', t11(697), 4); printMessage(0, 89, 0);
			modifyObjectFlag('c', t11(697), 4); l12(0, 89, 1);
		}
		else {
			if (currentCommandContains(80) && object_type_3_buffer[PLAYER_LOCATION] == 141 && !(isObjectFlagSet(80, 5)))
			{
...
int h34()
{
	...
	if (!currentCommandIsOneOf(9, 78, -1)) return 0; if (currentCommandContains(9)) { s31(); }
	else {
	...
	if (!(object_type_3_buffer[697] == 1 || currentCommandContains(78)))
	{
		printMessage(64, 814, 0);
	...
	if (currentCommandContains(4) && object_type_3_buffer[4] == 0 && !isItemAtLocation(98, -1)) { printMessage(64, 820, 0); } (*command_by_location_dispatch_table[536])();
}
int u31() {
...
} int d36()
{
	...
	{
		printMessage(76, 818, 670);
	} if (!currentCommandIsOneOf(111, 42, 49, 100, 106, 107, 118, 98, 93, -1)) return
	...
	{
		z27();
	} if (currentCommandContains(118)) { t37(); } if (currentCommandContains(98)) { printMessage(64, 842, 0); } if (currentCommandContains(73))
	{
	...
	} if (!(currentCommandContains(35) || isItemAtLocation(98, -1))) {
...
} int d33() { f3(700, 940); f3(701, 943); c28(); longjmp(done_with_command, 1); } int p32()
{
	...
	if (object_type_3_buffer[697] == 2) {
		if (isObjectFlagSet(t11(670), 8)) {
			...
			} if (currentCommandContains(10)) { (*command_by_location_dispatch_table[564])(); }
		}
	}
} int
n22() {
...
a33() { l12(0, 714, 1); printMessage(66, 719, 0); } int g28() {
	...
	if (object_type_3_buffer[697] == 2 && !currentCommandContains(493))
	{
	...
	if (!currentCommandContains(53)) return; if (!g10(53, -1)) return 0; s9(53, 485);
...
int n31() {
	...
	if (g10(110, -1))
	{
		if (currentCommandContains(110) || object_type_3_buffer[697] == 1) { s9(110, 484); printMessage(64, 868, 0); }
	} if (g10(119, -1))
	{
	...
	if (object_type_3_buffer[697] > 1) {
		if (currentCommandContains(88)) {
			...
			if (g10(t11(object_type_3_buffer[670]), -1)) {
				...
				{
					printMessage(64, 1255, 0);
				} if (currentCommandContains(660) && object_type_3_buffer[PLAYER_LOCATION] == 434 || currentCommandContains(71)) { u23(); } if (currentCommandContains(70))
				{
...
} int y19()
{
	...
	if (currentCommandContains(60)) {
		...
		s9(28, 484); processMoveCommand(239, -1163);
